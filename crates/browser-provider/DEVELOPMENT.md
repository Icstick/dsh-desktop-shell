# Browser Provider Development Contract

## Boundary

启动和管理 Chromium/Edge/CDP provider、profiles、sessions 与 human takeover。 它只处理本模块列出的 ownership，不通过便利性绕过上层 policy、下层 abstraction 或 Adapter boundary。
...
无 raw CDP、secret/autofill human-only、domain/action scope。
