# Browser Agent Adapter Development Contract

## Boundary

把 DSH browser tools 映射到 scoped Browser capability。 它只处理本模块列出的 ownership，不通过便利性绕过上层 policy、下层 abstraction 或 Adapter boundary。

## Data flow

```text
DSH tool calls + Browser agreement
...
mutation 默认拒绝；domain/action/owner scope 和 takeover。
