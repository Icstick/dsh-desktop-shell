# Legacy DSH Adapter Development Contract

## Boundary

吸收 DSH/Cordis 变化并映射内部 capability。 它只处理本模块列出的 ownership，不通过便利性绕过上层 policy、下层 abstraction 或 Adapter boundary。
...
所有 Agent action 保留 DSH policy/provenance；process token 非插件身份。
