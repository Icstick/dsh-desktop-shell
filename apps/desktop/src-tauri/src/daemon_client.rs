...
//!   retries startup until the first successful connection and then
//!   stays; commands fail closed (UNAVAILABLE) if the daemon dies.
...
enum ClientCommand {
    Invoke {
        envelope: Envelope,
        reply: mpsc::SyncSender<Envelope>,
    },
}
...
impl DaemonConnector for DaemonClient {
    fn invoke(
        ...
        let (reply_tx, reply_rx) = mpsc::sync_channel::<Envelope>(1);
        self.commands
...
fn worker_loop(
    ...
    let mut pending: HashMap<String, mpsc::SyncSender<Envelope>> = HashMap::new();
            ...
            match commands.try_recv() {
                Ok(ClientCommand::Invoke { envelope, reply }) => {
                    ...
                    if transport.send_json(&envelope).is_err() {
                        let _ = reply.send(closed_result(&id));
                        break 'outer;
                    }
...
/// A synthetic Result envelope answering a send that failed before the
/// invocation left the client (the invoke side sees a transport error).
fn closed_result(id: &str) -> Envelope {
    Envelope {
        protocol: PROTOCOL.into(),
        id: new_message_id(),
        ...
        method: None,
        payload: None,
        error: Some(dsh_daemon::envelope::ProtocolError {
            code: ErrorCode::Unavailable,
            message: "client transport closed before the invocation was sent".into(),
            retryable: true,
...
/// successful connection, then bridge events and return. The Shell never
/// disconnects the daemon on close (M6 core semantics); if the daemon
/// dies later, commands fail closed until a future reconnect slice.
