# Usage UI Development Contract

## Boundary

展示 DSH 侧标准化的 usage、来源、范围和估算标记。 它只处理本模块列出的 ownership，不通过便利性绕过上层 policy、下层 abstraction 或 Adapter boundary。
...
不显示 session prompt/body；成本必须标识 estimate/source。
