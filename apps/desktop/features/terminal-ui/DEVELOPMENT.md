# Terminal UI Development Contract

## Boundary

显示和操作 Supervisor-owned Human Terminal sessions。 它只处理本模块列出的 ownership，不通过便利性绕过上层 policy、下层 abstraction 或 Adapter boundary。
...
Human Surface 不自动授予 Agent Automation。
